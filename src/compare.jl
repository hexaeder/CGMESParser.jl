"""
    CIMCollectionComparison(A::CIMCollection, B::CIMCollection)

Match up the objects of two collections, so that two exports of the same grid can be
compared. Objects are matched on their class, properties and what they reference — not on
UUID, which differs between exports.

The result holds the matching in both directions. Objects that are genuinely identical to
one another (duplicate limits, position points) can be matched ambiguously; that is warned
about but not an error.
"""
...
# Two references count as equal when they point at the same thing. Names are the usable
# identity here, since the UUIDs differ between two exports of the same grid. Classes like
# PositionPoint carry no name at all and fall back to their class.
function equal_property(a::CIMRef, b::CIMRef)
    !a.resolved && !b.resolved && return true # ignore unresolved references
    a.resolved && b.resolved || return false

    ta, tb = follow_ref(a), follow_ref(b)
    isnothing(ta) && isnothing(tb) && return true
    (isnothing(ta) || isnothing(tb)) && return false

    if hasname(ta) && hasname(tb)
        return getname(ta) == getname(tb)
    end
    return !hasname(ta) && !hasname(tb) && ta.class_name == tb.class_name
end
...
"""
    compare_objects(obj1, obj2; io=stdout)

Print the properties of two objects side by side, marking what differs.
"""
